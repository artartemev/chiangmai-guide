...
print("Parsing result 5.json (ru_chiangmai)...")
...
print(f"Found {len(dimas_data)} user IDs in ru_chiangmai.")
...
async def main():
    ...
    <body class="p-8">
        <h1 class="text-2xl text-[#ff2e2e] mb-2"><i class="fa-solid fa-users"></i> СПИСОК ВСЕХ ДИМ</h1>
        <p class="text-gray-400 mb-6">Из чата ru_chiangmai (Чиангмай и Северный Таиланд)</p>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    """
    
    fetched = 0
    # Group message IDs to fetch in bulk if possible, or fetch one by one
    for uid, info in dimas_data.items():
        try:
            # Fetch the specific message by ID so Telethon caches the user entity
            msg = await client.get_messages('ru_chiangmai', ids=info["msg_id"])
            ...
            pass # Ignore and fallback to JSON data
    ...
    print(f"Successfully generated HTML with {len(dimas_data)} profiles ({fetched} enriched via msg fetch).")
