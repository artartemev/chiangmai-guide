...
async def main():
    ...
    print("Connected. Fetching messages from 'ru_chiangmai'...")
    
    dimas = {}
    search_queries = ["дима", "дмитрий", "dima", "dmitry", "dmitriy"]
    
    # We will iterate through up to 50,000 recent messages
    count = 0
    try:
        async for msg in client.iter_messages('ru_chiangmai', limit=50000):
            count += 1
            if count % 5000 == 0:
            ...
            if sender and getattr(sender, 'first_name', None):
                name_parts = []
                if sender.first_name: name_parts.append(sender.first_name)
                if sender.last_name: name_parts.append(sender.last_name)
                full_name = " ".join(name_parts)
                
                name_lower = full_name.lower()
                username_lower = sender.username.lower() if sender.username else ""
                
                if any(q in name_lower for q in search_queries) or any(q in username_lower for q in search_queries):
                    if sender.id not in dimas:
                        dimas[sender.id] = sender
    except Exception as e:
        print(f"Error: {e}")

    print(f"Total Dimas found: {len(dimas)}")
    ...
    <head>
        ...
        <title>Список Дим // ru_chiangmai</title>
    ...
    <body class="p-8">
        <h1 class="text-2xl text-[#ff2e2e] mb-6">Список всех Дим (ru_chiangmai)</h1>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    """
    
    for user_id, user in dimas.items():
        name_parts = []
